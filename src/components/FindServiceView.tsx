...
export const FindServiceView: React.FC<FindServiceViewProps> = ({
  ...
  const popularIssues = [
    ...
    { titleHi: 'दरवाजे का लॉक जाम है', titleEn: 'Door lock jammed / repair', catId: 15, icon: '🔑' },
                  ...
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      ...
                      <span className="text-[10px] text-slate-500 font-medium">
                        {language === 'hi' ? matchedCat.nameHi : matchedCat.name} • {language === 'hi' ? `अनुमानित ${matchedCat.avgRate || '₹249'}` : `Estimated ${matchedCat.avgRate || '₹249'}`}
                      </span>
                    </div>
                  </div>
      ...
      {noResultsFound && (
        <div id="no-results-box" className="bg-white rounded-3xl border border-slate-200 p-8 text-center space-y-3 shadow-sm">
          ...
          <p className="text-xs text-slate-500 max-w-xs mx-auto">
            {language === 'hi'
              ? `"${searchQuery}" के लिए कोई सर्विस या कारीगर नहीं मिला। कृपया दूसरा शब्द खोजें या सभी श्रेणियां देखें।`
              : `No services or professionals found matching "${searchQuery}". Try a different keyword or browse all categories.`}
          </p>
          <div className="pt-2 flex flex-wrap items-center justify-center gap-2">
            <button
              id="clear-search-no-results-btn"
              onClick={() => onSearchChange('')}
              className="px-4 py-2 bg-amber-500 hover:bg-amber-600 text-white text-xs font-bold rounded-xl cursor-pointer shadow-xs transition-colors"
            >
                  ...
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-slate-500 mt-0.5">
                        {subName} • <span className="text-emerald-700 font-bold">{language === 'hi' ? `अनुमानित: ${item.service.avgRate || '₹250+'}` : `Estimated: ${item.service.avgRate || '₹250+'}`}</span>
                      </p>
                    </div>
                  </div>
            ...
            {filteredCategories.slice(0, 8).map((cat) => {
              return (
                ...
                >
                  ...
                  <span className="text-[10px] text-slate-400 font-medium mt-0.5">{language === 'hi' ? `अनुमानित ${cat.avgRate || '₹250'}` : `Estimated ${cat.avgRate || '₹250'}`}</span>
                </div>
              );
            })}
          </div>
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 text-[10px] text-slate-500 mt-0.5">
                        ...
                        <span>•</span>
                        <span className="font-bold text-slate-700">{language === 'hi' ? `₹${p.hourlyRate} से शुरू / घंटा` : `Starting from ₹${p.hourlyRate}/hr`}</span>
                      </div>
                    </div>
                  </div>
